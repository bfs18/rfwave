...
class RectifiedFlow(nn.Module):
    ...
    def get_pred(self, z_t, t, mel, reference, bandwidth_id, encodec_bandwidth_id=None):
        pred = self.backbone(z_t, t, mel, reference, bandwidth_id, encodec_bandwidth_id)
        return pred

    @torch.no_grad()
    def sample_ode_subband(self, mel, reference, band, bandwidth_id,
        ...
        if self.prev_cond or not self.parallel_uncond:
            ...
            mel = torch.repeat_interleave(mel, self.num_bands, 0)
            reference = torch.repeat_interleave(reference, self.num_bands, 0)
        ...
        for i in range(len(ts) - 1):
            ...
            if self.cfg:
                mel_ = torch.cat(
                    [mel, torch.ones_like(mel) * mel.mean(dim=(2,), keepdim=True)], dim=0)
                reference_ = torch.cat(
                    [reference, torch.ones_like(reference) * reference.mean(dim=(2,), keepdim=True)], dim=0)
                (z_, t_, bandwidth_id_) = [torch.cat([v] * 2, dim=0) for v in (z, t, bandwidth_id)]
                pred = self.get_pred(z_, t_, mel_, reference_, bandwidth_id_, encodec_bandwidth_id)
                pred, uncond_pred = torch.chunk(pred, 2, dim=0)
                pred = uncond_pred + self.guidance_scale * (pred - uncond_pred)
            else:
                pred = self.get_pred(z, t, mel, reference, bandwidth_id, encodec_bandwidth_id)
            if self.wave:
                if self.prev_cond or not self.parallel_uncond:
    ...
    def sample_ode(self, mel, reference, encodec_bandwidth_id=None, N=None, ts=None, keep_traj=False):
        traj = []
        if self.prev_cond or not self.parallel_uncond:
            ...
            for i in range(self.num_bands):
                ...
                traj_i = self.sample_ode_subband(
                    mel, reference, band, bandwidth_id, encodec_bandwidth_id=encodec_bandwidth_id,
    ...
    def compute_loss(self, z_t, t, target, mel, reference, bandwidth_id, encodec_bandwidth_id=None):
        if self.cfg and np.random.uniform() < self.p_uncond:
            mel = torch.ones_like(mel) * mel.mean(dim=(2,), keepdim=True)
        pred = self.get_pred(z_t, t, mel, reference, bandwidth_id, encodec_bandwidth_id=encodec_bandwidth_id)
        ...
        return loss + (stft_loss + phase_loss + overlap_loss) * 0.01, loss_dict
...
class VocosExp(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx, **kwargs):
        audio_input, reference, num_reference_frames = batch
        features = self.feature_extractor(audio_input, **kwargs)
        reference = self.reflow.stft(reference)
        # train generator
        ...
        reference_ext = torch.repeat_interleave(reference, self.num_bands, 0)
        loss, loss_dict = self.reflow.compute_loss(
            z_t, t, target, features_ext, reference_ext, bandwidth_id=bandwidth_id)
        ...
        if self.global_step % 1000 == 0 and self.global_rank == 0:
            with torch.no_grad():
                audio_hat_traj = self.reflow.sample_ode(features, reference, N=100, **kwargs)
            audio_hat = audio_hat_traj[-1]
            self.logger.log_metrics(
                {"train/total_loss": loss}, step=self.global_step)
    ...
    def on_validation_epoch_end(self, **kwargs):
        ...
        if self.global_rank == 0:
            ...
            metrics = {"valid/phase_loss": phase_loss}
