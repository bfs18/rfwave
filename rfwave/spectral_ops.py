...
class STFT(nn.Module):
    ...
    def forward(self, y: torch.Tensor) -> torch.Tensor:
        if self.padding == "center":
            ...
            return torch.stft(
                y, self.n_fft, self.hop_length, self.win_length, self.window, center=True, return_complex=True)
        elif self.padding == "same":
            pad = (self.win_length - self.hop_length) // 2
        else:
            raise ValueError("Padding must be 'center' or 'same'.")
        y_p = torch.nn.functional.pad(y, (pad, pad))
        y_f = y_p.unfold(1, self.win_length, self.hop_length)
        y_w = y_f * self.window
        spec = torch.fft.rfft(y_w, self.n_fft, dim=2, norm='backward')
